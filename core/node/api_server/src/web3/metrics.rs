...
// toggled (histogram vs rolling-summary) without touching `status`. `status` is always
// registered; the latency count comes from the histogram in default mode, and from an explicit
// counter in summary mode (the histogram is filtered out then).
#[derive(Debug, Metrics)]
...
// `leafage_rpc_call_time_count` as a persistent counter — collected only in summary mode, because
// the histogram (which normally supplies this series) is filtered out.
// NOTE: this struct name is matched as a string literal by the exporter filter in
// `zksync_vlog::prometheus`; keep it in sync with that filter.
#[derive(Debug, Metrics)]
#[metrics(prefix = "leafage_rpc_call")]
pub(crate) struct LeafageRpcTimeSummaryCount {
    pub time_count: Family<MethodNameLabel, Counter>,
}

#[vise::register]
pub(crate) static LEAFAGE_RPC_TIME_SUMMARY_COUNT_METRICS: vise::Global<LeafageRpcTimeSummaryCount> =
    vise::Global::new();
