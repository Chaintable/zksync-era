...
mod tests {
    ...
    #[test]
    fn validates_rpc_settlement_layer_chain_ids() {
        let layer = RpcSettlementLayerData::new(L1ChainId(1), Some(SLChainId(270)));
        layer
            .validate_settlement_layer(SettlementLayer::L1(SLChainId(1)))
            .unwrap();
        layer
            .validate_settlement_layer(SettlementLayer::Gateway(SLChainId(270)))
            .unwrap();

        let err = layer
            .validate_settlement_layer(SettlementLayer::L1(SLChainId(9)))
            .unwrap_err();
        assert!(err.to_string().contains("configured L1 chain ID"), "{err}");

        let err = layer
            .validate_settlement_layer(SettlementLayer::Gateway(SLChainId(271)))
            .unwrap_err();
        assert!(
            err.to_string().contains("configured Gateway chain ID"),
            "{err}"
        );
    }

    #[test]
    fn rejects_gateway_without_configured_chain_id() {
        let layer = RpcSettlementLayerData::new(L1ChainId(1), None);
        let err = layer
            .validate_settlement_layer(SettlementLayer::Gateway(SLChainId(270)))
            .unwrap_err();
